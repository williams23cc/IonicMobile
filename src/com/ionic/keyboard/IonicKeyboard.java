...
public class IonicKeyboard extends CordovaPlugin {
	...
	public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		// http://stackoverflow.com/a/4737265/1091751 detect if keyboard is
		// showing
		final View rootView = cordova.getActivity().getWindow().getDecorView()
		...
		OnGlobalLayoutListener list = new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				if (pixelHeightDiff > 100
						&& pixelHeightDiff != previousHeightDiff) { // if more
																	// than 100
																	// pixels,
																	// its
																	// probably
					...
					appView.sendJavascript("cordova.fireWindowEvent('native.keyboardshow', { 'keyboardHeight':"
							+ Integer.toString(pixelHeightDiff) + "});");

					// deprecated
					appView.sendJavascript("cordova.fireWindowEvent('native.showkeyboard', { 'keyboardHeight':"
							+ Integer.toString(pixelHeightDiff) + "});");
	...
	public boolean execute(String action, JSONArray args,
		...
		if ("close".equals(action)) {
			cordova.getThreadPool().execute(new Runnable() {
				public void run() {
					...
					InputMethodManager inputManager = (InputMethodManager) cordova
		...
		if ("show".equals(action)) {
			cordova.getThreadPool().execute(new Runnable() {
				public void run() {
					((InputMethodManager) cordova.getActivity()
